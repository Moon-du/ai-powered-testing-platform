...
const stepReady = computed(() => {
  if (currentStep.value === 0) return Boolean(form.product_type_id)
  if (currentStep.value === 1) return Boolean(form.knowledge_pack_id)
...
const createMutation = useMutation({
  ...
  onSuccess: (project) => {
    void message.success('项目已创建并固定 Knowledge Pack 版本')
...
<template>
  ...
  <a-card class="surface-card wizard-card" :bordered="false">
    ...
    <section v-else-if="currentStep === 1">
      ...
      />
      <a-skeleton v-if="packsQuery.isPending.value" active />
      <div v-else class="choice-grid">
        <button
      ...
      <a-empty v-if="!packsQuery.isPending.value && !(packsQuery.data.value ?? []).length" description="该产品暂无可用 Published Pack" />
    </section>
    ...
    <section v-else>
      ...
      <a-descriptions bordered :column="{ xs: 1, md: 2 }">
        <a-descriptions-item label="Product Type">
          {{ selectedProductType?.display_name }} / {{ selectedProductType?.name }}
        </a-descriptions-item>
